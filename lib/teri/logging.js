...
// Optional: if you ever want to reduce volume (default 1.0 = log everything)
function getSampleRate() {
  const raw = process.env.TERI_LOG_SAMPLE_RATE;
 ...
 *
 * IMPORTANT: Logging must never be able to crash the function.
 */
export function logTeriEvent(event) {
  ...
  // Optionally send to Supabase sink (non-blocking, MUST be safe)
  ...
  // CRITICAL: attach a catch so aborts/network failures never become unhandled rejections
  fetch(url, {
    method: "POST",
    headers: {
    ...
    .catch((err) => {
      // Swallow errors; optionally keep a lightweight debug line in dev only
      if (process.env.NODE_ENV === "development") {
        console.warn("[teri-log-sink] failed:", err?.name || err);
      }
    })
    .finally(() => {
